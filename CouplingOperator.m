...
elseif find(strcmp('FE2D',Nmod))
    ...
    IntM = zeros(3*size(Int.M));
    IntM(1:size(Int.M,1),1:size(Int.M,2)) = Int.M;
    IntM(size(Int.M,1)+1:2*size(Int.M,1),size(Int.M,2)+1:2*size(Int.M,2)) = Int.M;
    IntM(2*size(Int.M,1)+1:3*size(Int.M,1),2*size(Int.M,2)+1:3*size(Int.M,2)) = Int.M;
