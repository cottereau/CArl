...
void set_homogeneous_physical_properties(libMesh::EquationSystems& es, std::string& physicalParamsFile)
{
	...
	else if (es.has_system("Dynamic"))
	{
		...
		{
			std::ifstream physicalParamsIFS(physicalParamsFile);
			physicalParamsIFS >> inputRho >> inputE >> inputMu;
		}
		...
		physical_consts[0] = physical_param_system.variable_number ("rho");
		physical_consts[1] = physical_param_system.variable_number ("E");
		physical_consts[2] = physical_param_system.variable_number ("mu");
	...
	{
		std::cout << "Le système d'équation n'a pas cette physique !" << std::endl;
	}
...
void Update_SubM(	libMesh::DenseSubMatrix<libMesh::Number>& SubM,
					unsigned int qp,  // numéro du point de Gauss
					const std::vector<std::vector<libMesh::Real> >& phi,
					const unsigned int n_u_dofs, // nombre de noeuds
					const std::vector<libMesh::Real>& JxW, // produit du jacobien et des poids
					libMesh::Number rho, // masse volumique
					double cte
				)
{
	for (unsigned int iii=0; iii<n_u_dofs; iii++)
...
void Update_SubC(	libMesh::DenseSubMatrix<libMesh::Number>& SubC,
					...
					const unsigned int n_u_dofs, // nombre de noeuds
					const std::vector<libMesh::Real>& JxW, // produit du jacobien et des poids
					libMesh::Number c, // masse volumique
					double cte
				)
{
	for (unsigned int iii=0; iii<n_u_dofs; iii++)
									        ...
									        WeightFunctionSystemType system_type)
{
	...
	libMesh::PerfLog perf_log ("Matrix Assembly (Homogeneous elasticity dynamic)",MASTER_bPerfLog_assemble_fem);
	...
	libMesh::NewmarkSystem& system = es.get_system<libMesh::NewmarkSystem>("Dynamic");
	...
	//**************************
	// Ajouts par rapport à la fonction d'assemblage statigue
	// à partir du second example dynamique donné par libmesh.
	...
	// -> 3D elements
	libMesh::UniquePtr<libMesh::FEBase> fe (libMesh::FEBase::build(dim, fe_type));
	libMesh::QGauss qrule (dim, fe_type.default_quadrature_order()); // Gauss quadrature
	fe->attach_quadrature_rule (&qrule); // Tell the finite element object to use our quadrature rule.
	...
	// Jacobian
	const std::vector<libMesh::Real>& JxW = fe->get_JxW(); // The element Jacobian * quadrature weight at each integration point.
	...
	// The element mass, damping and stiffness matrices
  	...
  	libMesh::DenseMatrix<libMesh::Number> Ke, Ce, Me;
	...
	for ( ; el != end_el; ++el)
	{
		...
		*/
		...
		for (unsigned int qp=0; qp<qrule.n_points(); qp++)
		{
			...
			// Mass matrix
			Update_SubM(Muu, qp, phi, n_u_dofs, JxW, localRho, weight_alpha);
			Update_SubM(Mvv, qp, phi, n_u_dofs, JxW, localRho, weight_alpha);
			Update_SubM(Mww, qp, phi, n_u_dofs, JxW, localRho, weight_alpha);
			...
			// Damping matrix
			/*
			Update_SubC(Cuu, qp, phi, n_u_dofs, JxW, 0, weight_alpha);
			Update_SubC(Cuv, qp, phi, n_u_dofs, JxW, 0, weight_alpha);
			Update_SubC(Cuw, qp, phi, n_u_dofs, JxW, 0, weight_alpha);
			
			Update_SubC(Cvu, qp, phi, n_u_dofs, JxW, 0, weight_alpha);
			Update_SubC(Cvv, qp, phi, n_u_dofs, JxW, 0, weight_alpha);
			Update_SubC(Cvw, qp, phi, n_u_dofs, JxW, 0, weight_alpha);
			
			Update_SubC(Cwu, qp, phi, n_u_dofs, JxW, 0, weight_alpha);
			Update_SubC(Cwv, qp, phi, n_u_dofs, JxW, 0, weight_alpha);
			Update_SubC(Cww, qp, phi, n_u_dofs, JxW, 0, weight_alpha);
			*/
		...
		dof_map.constrain_element_matrix(Ce, dof_indices);
		perf_log.pop("Constraints","Matrix element calculations");

		perf_log.push("Adding elements");
		//system.matrix->add_matrix (Me, dof_indices);
		//system.matrix->add_matrix (Ke, dof_indices);
		//system.matrix->add_matrix (Ce, dof_indices);
