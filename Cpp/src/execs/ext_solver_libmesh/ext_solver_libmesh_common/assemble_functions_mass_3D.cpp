...
void Update_SubM( libMesh::DenseSubMatrix<libMesh::Number>& SubM,
  ...
  const unsigned int n_components,const unsigned int n_u_dofs,const unsigned int n_v_dofs,
  const std::vector<libMesh::Real>& JxW, double weight)
{
  for (unsigned int iii=0; iii<n_u_dofs; iii++)
  {
    for (unsigned int jjj=0; jjj<n_v_dofs; jjj++)
    {
  ...
  double CK,double CM)
{
  for (unsigned int iii=0; iii<n_u_dofs; iii++)
  {
    for (unsigned int jjj=0; jjj<n_v_dofs; jjj++)
    {
      SubC(iii,jjj) += CK*SubK(iii,jjj)+CM*SubM(iii,jjj);
    }
  }
}
...
void assemble_dynamic_elasticity_with_weight(libMesh::EquationSystems& es,
  ...
  WeightFunctionSystemType system_type, carl::NewmarkParams* newmark,double CM,double CK)
{
  ...
  PetscScalar betaDeltat2 = (1-newmark->alpha)*(newmark->beta)*(newmark->deltat)*(newmark->deltat);
  PetscScalar gammaDeltat = (newmark->gamma)*(newmark->deltat);
  ...
  for ( ; el != end_el; ++el)
  {
    ...
    Kuv.reposition (u_var*n_u_dofs, v_var*n_v_dofs, n_u_dofs, n_v_dofs);
    Kuw.reposition (u_var*n_u_dofs, w_var*n_w_dofs, n_u_dofs, n_w_dofs);

    Kvu.reposition (v_var*n_v_dofs, u_var*n_u_dofs, n_v_dofs, n_u_dofs);
    Kvv.reposition (v_var*n_v_dofs, v_var*n_v_dofs, n_v_dofs, n_v_dofs);
    Kvw.reposition (v_var*n_v_dofs, w_var*n_w_dofs, n_v_dofs, n_w_dofs);

    Kwu.reposition (w_var*n_w_dofs, u_var*n_u_dofs, n_w_dofs, n_u_dofs);
    Kwv.reposition (w_var*n_w_dofs, v_var*n_v_dofs, n_w_dofs, n_v_dofs);
    Kww.reposition (w_var*n_w_dofs, w_var*n_w_dofs, n_w_dofs, n_w_dofs);

    // Set the positions of the sub-matrices
    Cuu.reposition (u_var*n_u_dofs, u_var*n_u_dofs, n_u_dofs, n_u_dofs);
    Cuv.reposition (u_var*n_u_dofs, v_var*n_v_dofs, n_u_dofs, n_v_dofs);
    Cuw.reposition (u_var*n_u_dofs, w_var*n_w_dofs, n_u_dofs, n_w_dofs);

    Cvu.reposition (v_var*n_v_dofs, u_var*n_u_dofs, n_v_dofs, n_u_dofs);
    Cvv.reposition (v_var*n_v_dofs, v_var*n_v_dofs, n_v_dofs, n_v_dofs);
    Cvw.reposition (v_var*n_v_dofs, w_var*n_w_dofs, n_v_dofs, n_w_dofs);

    Cwu.reposition (w_var*n_w_dofs, u_var*n_u_dofs, n_w_dofs, n_u_dofs);
    Cwv.reposition (w_var*n_w_dofs, v_var*n_v_dofs, n_w_dofs, n_v_dofs);
    Cww.reposition (w_var*n_w_dofs, w_var*n_w_dofs, n_w_dofs, n_w_dofs);
    
    // Set the positions of the sub-matrices
    Muu.reposition (u_var*n_u_dofs, u_var*n_u_dofs, n_u_dofs, n_u_dofs);
    Muv.reposition (u_var*n_u_dofs, v_var*n_v_dofs, n_u_dofs, n_v_dofs);
    Muw.reposition (u_var*n_u_dofs, w_var*n_w_dofs, n_u_dofs, n_w_dofs);

    Mvu.reposition (v_var*n_v_dofs, u_var*n_u_dofs, n_v_dofs, n_u_dofs);
    Mvv.reposition (v_var*n_v_dofs, v_var*n_v_dofs, n_v_dofs, n_v_dofs);
    Mvw.reposition (v_var*n_v_dofs, w_var*n_w_dofs, n_v_dofs, n_w_dofs);

    Mwu.reposition (w_var*n_w_dofs, u_var*n_u_dofs, n_w_dofs, n_u_dofs);
    Mwv.reposition (w_var*n_w_dofs, v_var*n_v_dofs, n_w_dofs, n_v_dofs);
    ...
    for (unsigned int qp=0; qp<qrule.n_points(); qp++)
    {
      ...
      Update_SubM(Muu, qp, phi, dim, n_u_dofs, n_u_dofs, JxW, localRho*weight_alpha);
      Update_SubM(Mvv, qp, phi, dim, n_v_dofs, n_v_dofs, JxW, localRho*weight_alpha);
      Update_SubM(Mww, qp, phi, dim, n_w_dofs, n_w_dofs, JxW, localRho*weight_alpha);

      Update_SubC(Cuu,Kuu,Muu,n_u_dofs, n_u_dofs, CK,CM);
      Update_SubC(Cuv,Kuv,Muu,n_u_dofs, n_v_dofs, CK,CM);
      Update_SubC(Cuw,Kuw,Muu,n_u_dofs, n_w_dofs, CK,CM);

      Update_SubC(Cvu,Kvu,Mvv,n_v_dofs, n_u_dofs, CK,CM);
      Update_SubC(Cvv,Kvv,Mvv,n_v_dofs, n_v_dofs, CK,CM);
      Update_SubC(Cvw,Kvw,Mvv,n_v_dofs, n_w_dofs, CK,CM);

      Update_SubC(Cwu,Kwu,Mww,n_w_dofs, n_u_dofs, CK,CM);
      Update_SubC(Cwv,Kwv,Mww,n_w_dofs, n_v_dofs, CK,CM);
      Update_SubC(Cww,Kww,Mww,n_w_dofs, n_w_dofs, CK,CM);
...
void assemble_dynamic_elasticity_with_weight_and_traction(libMesh::EquationSystems& es,
                ...
                carl::NewmarkParams* newmark,double CM,double CK)
{
...
// Dynamic elasticity with bending force
void assemble_dynamic_elasticity_with_weight_and_bending(libMesh::EquationSystems& es, const std::string& system_name,
weight_parameter_function& weight_mask, WeightFunctionSystemType system_type,
int traction_boundary_id, std::vector<double> traction_density, carl::NewmarkParams* newmark,double CM,double CK){
  ...
  for ( ; el != end_el; ++el)
  {
    ...
    Fw.reposition (w_var*n_w_dofs, n_w_dofs);
    ...
    dof_map.constrain_element_matrix(Ce, dof_indicesC);
    dof_map.constrain_element_matrix(Ke, dof_indicesM);
    perf_log.pop("Constraints","Matrix element calculations");
