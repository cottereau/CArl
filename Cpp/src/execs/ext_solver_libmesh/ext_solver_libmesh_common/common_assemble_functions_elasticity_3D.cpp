...
{
    _u_var = this->add_variable ("u_disp", libMesh::FIRST, libMesh::LAGRANGE);
    _v_var = this->add_variable ("v_disp", libMesh::FIRST, libMesh::LAGRANGE);
    _w_var = this->add_variable ("w_disp", libMesh::FIRST, libMesh::LAGRANGE);
...

{
    ...
    libMesh::Real p_value;
    if (this->time < .002)
        p_value = sin(2*pi*this->time/.002);
    else
        p_value = .0;
    ...
    for (unsigned int qp=0; qp != n_qpoints; qp++)
    {
        ...
        for (unsigned int i=0; i != n_u_dofs; i++)
        {
            ...
            {
                Fu(i) += (tau(0,alpha)*grad_phi[i][qp](alpha) - p_value*body_force(0)*phi[i][qp])*JxW[qp];
                Fv(i) += (tau(1,alpha)*grad_phi[i][qp](alpha) - p_value*body_force(1)*phi[i][qp])*JxW[qp];
                Fw(i) += (tau(2,alpha)*grad_phi[i][qp](alpha) - p_value*body_force(2)*phi[i][qp])*JxW[qp];
        ...
        unsigned int k, unsigned int l)
{
    ...
    const libMesh::Real young_modulus = 1.0e13;
...
void set_clamped_border_dyn(ElasticitySystem& elasticity_system, int boundary_id)
{
    ...
    std::vector<unsigned int> variables(3);
    variables[0] = elasticity_system.variable_number("u_disp");
    variables[1] = elasticity_system.variable_number("v_disp");
    variables[2] = elasticity_system.variable_number("w_disp");
