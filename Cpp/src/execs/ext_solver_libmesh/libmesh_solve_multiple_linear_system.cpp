#include "libmesh_solve_multiple_linear_system.h"
...
int main(int argc, char** argv) {
  ...
  carl::libmesh_solve_multiple_linear_system_input_params input_params;
  ...
  std::string rhs_vector;
  std::string output_vector;
  
  // Read
  for(int kkk=input_params.number_begin; kkk<=input_params.number_end ;kkk++){

    std::cout << " Solving " << kkk << "..." << std::endl;

    rhs_vector=input_params.sys_rhs_vec_file;
    output_vector=input_params.output_base;

    rhs_vector = rhs_vector.replace(rhs_vector.find("*"),1,std::to_string(kkk));
    output_vector = output_vector.replace(output_vector.find("*"),1,std::to_string(kkk));

    carl::read_PETSC_matrix(sys_mat_PETSC, input_params.sys_matrix_file, WorldComm.get());
    carl::read_PETSC_vector(sys_rhs_vec_PETSC, rhs_vector, WorldComm.get());
  ...
  // Print MatLab debugging output? Variable defined at "carl_headers.h"
    ...
    sys_sol_vec.print_matlab(output_vector + "_sys_sol_vec.m");
  #endif

    // Export the solution vector
    carl::write_PETSC_vector(sys_sol_vec, output_vector + "_sys_sol_vec.petscvec");
