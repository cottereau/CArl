...
/** \brief Program used to assemble the right-hand side for a dynamic system with a clamped \f$x_{\mbox{Max}}\f$ face.
 * 
 *  Usage: `./libmesh_assemble_rhs_dyn__max_x_dirichlet -i [input file]`
 *  
 */
...
int main(int argc, char** argv) {
  ...
  set_state(equation_systems,
            "Dynamic",
            input_params.scratch_folder_path + "/domain_B_state/init_cond_B_disp.petscvec",
            input_params.scratch_folder_path + "/domain_B_state/init_cond_B_vel.petscvec",
            input_params.scratch_folder_path + "/domain_B_state/init_cond_B_acc.petscvec");
 ...
 // Print MatLab debugging output? Variable defined at "carl_headers.h"
  ...
  carl::write_PETSC_vector(*RHS_ptr,  input_params.scratch_folder_path + "/domain_B_state/traction_model_B_sys_rhs_vec.petscvec");
      ...
      if (std::abs(curr_node(2)-z_coo) < 1.e-6)
        {
          ...
          // Here we apply sinusoidal displactement throug u values for 0<t<0.002
          // at one end of the barre-mesh.
          libMesh::Real w_value;
          if (t_system.time < (2*pi/17500))
          {
            w_value = sin(17500*t_system.time);
            std::cout << "t_system.time = " << t_system.time << std::endl;
