/*
 * libmesh_solve_multiple_linear_system_input_parser.h
 *
...
void get_input_params(GetPot& field_parser,
    libmesh_solve_multiple_linear_system_input_params& input_params) {

  if (field_parser.search(1, "SysMatrix")) {
    input_params.sys_matrix_file = field_parser.next(
        input_params.sys_matrix_file);
  } else {
    homemade_error_msg("[CArl Error]Missing the system matrix file!");
  }

  if (field_parser.search(1, "NumBegin")) {
    input_params.number_begin = field_parser.next(
        input_params.number_begin);
  } else {
    homemade_error_msg("[CArl Error]Missing number begin line!");
  }

  if (field_parser.search(1, "NumEnd")) {
    input_params.number_end = field_parser.next(
        input_params.number_end);
  } else {
    homemade_error_msg("[CArl Error]Missing number end line!");
  }
  ...
  if (field_parser.search(1, "SysRHSVector")) {
    ...
    homemade_error_msg("[CArl Error]Missing the system RHS vector file!");
  }
  ...
  if (field_parser.search(1, "OutputBase")) {
    ...
    homemade_error_msg("[CArl Error]Missing the output filename base!");
  }
...
void print_input_params(const std::string& output_filename,
  ...
  output_file << "NumBegin " << input_params.number_begin << std::endl;
  output_file << "NumEnd " << input_params.number_end << std::endl;
