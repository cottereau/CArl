...
namespace carl
{
  ...
  void Intersection_Search::FindPatchIntersections_Brute(const libMesh::Elem  * Query_elem)
  {
    ...
    for(  it_patch_A =  Patch_Set_A.begin();
        ...
        ++it_patch_A)
    {
      const libMesh::Elem * elem_A = m_Mesh_A.elem_ptr(*it_patch_A);
          ...
          ++it_patch_B)
      {
        ...
        ++nbOfTests;
        const libMesh::Elem * elem_B = m_Mesh_B.elem_ptr(*it_patch_B);
              ...
              std::pair<unsigned int,unsigned int> &    First_intersection)
  {
    ...
    for(std::set<unsigned int>::iterator it_inter = Intersecting_guide_elems.begin();
        ...
        ++it_inter)
    {
      Patch_guide_first_elem = Mesh_patch_guide.elem_ptr(*it_inter);
                  ...
                  std::pair<unsigned int,unsigned int> &    First_intersection)
  {
    ...
    for(  it_patch_Guide =  Patch_Set_Guide.begin();
        ...
        ++it_patch_Guide)
    {
      const libMesh::Elem * elem_Guide = Mesh_patch_guide.elem_ptr(*it_patch_Guide);

      for(  it_patch_Probed =  Patch_Set_Probed.begin();
          it_patch_Probed != Patch_Set_Probed.end();
          ++it_patch_Probed)
      {
        const libMesh::Elem * elem_Probed = Mesh_patch_probed.elem_ptr(*it_patch_Probed);
  ...
  void Intersection_Search::CalculateIntersectionVolume(const libMesh::Elem   * Query_elem)
  {
        ...
        ++it_patch_A)
    {
      ...
      for(auto it_patch_B = iterator_pair.first ;
           ...
           ++it_patch_B )
      {
        const libMesh::Elem * elem_B = m_Mesh_B.elem_ptr(it_patch_B->second);
        points_out.clear();
