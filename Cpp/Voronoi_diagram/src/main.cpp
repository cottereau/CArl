...
//	int dummy_nnn = 5000;
//	std::vector<double> faces(dummy_nnn,0);
//	std::vector<double> edges(dummy_nnn,0);
//
//	for(int nnn = 0; nnn < dummy_nnn; ++nnn)
//	{
//		con.clear();
//
//		// Randomly add particles into the container
...
//		}
//
//		voro_statistics(con,faces[nnn],edges[nnn]);
//	}
//
//	double sum = std::accumulate(faces.begin(), faces.end(), 0.0);
//	double mean = sum / faces.size();
//
//	double sq_sum = std::inner_product(faces.begin(), faces.end(), faces.begin(), 0.0);
//	double stdev = std::sqrt(sq_sum / faces.size() - mean * mean);
//
//	std::cout << "Faces : " << mean << " " << stdev << std::endl;
//
//	sum = std::accumulate(edges.begin(), edges.end(), 0.0);
//	mean = sum / edges.size();
//
//	sq_sum = std::inner_product(edges.begin(), edges.end(), edges.begin(), 0.0);
//	stdev = std::sqrt(sq_sum / edges.size() - mean * mean);
//
//	std::cout << "Edges : " << mean << " " << stdev << std::endl;
}
