		...
		)
{
	...
	/*
	 * 			Unordered set used to save all the intersection pairs already
	 * 		considered. It is used to avoid the "already treated" bug that
	 * 		appears when non-convex meshes are used.
	 */
	std::unordered_set<std::pair<int, int>, boost::hash<std::pair<int, int> > >
			treated_pairs(dtA.get_nb_of_cells()*dtB.get_nb_of_cells());
	std::pair<int, int> tetra_pair;
