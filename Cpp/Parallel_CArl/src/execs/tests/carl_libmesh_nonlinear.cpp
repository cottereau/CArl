#include "carl_libmesh_nonlinear.h"
...
struct carl_coupling_generation_input_params {
	...
	int nb_of_nonlinear_solves;
	double nonlinear_abs_tol;
	double nonlinear_rel_tol;
	unsigned int nb_of_nonlinear_max_its;
};
...
//		}
	...
	if (field_parser.search(1, "NonlinearSteps")) {
		input_params.nb_of_nonlinear_solves = field_parser.next(
				input_params.nb_of_nonlinear_solves);
	}
	else
	{
		input_params.nb_of_nonlinear_solves = 5;
	}

	if (field_parser.search(1, "NonlinearRelTol")) {
		input_params.nonlinear_rel_tol = field_parser.next(
				input_params.nonlinear_rel_tol);
	}
	else
	{
		input_params.nonlinear_rel_tol =  1.e-6;
	}

	if (field_parser.search(1, "NonlinearAbsTol")) {
		input_params.nonlinear_abs_tol = field_parser.next(
				input_params.nonlinear_abs_tol);
	}
	else
	{
		input_params.nonlinear_abs_tol =  1.e-6;
	}

	if (field_parser.search(1, "NonlinearMaxIts")) {
		input_params.nb_of_nonlinear_max_its = field_parser.next(
				input_params.nb_of_nonlinear_max_its);
	}
	else
	{
		input_params.nb_of_nonlinear_max_its = 50;
	}
}
;
...
int main(int argc, char** argv) {
	...
	boundary_displacement y_max_BIG(0,0.0001,0);
	boundary_displacement y_min_BIG(-0.025,0,0);
...
//	}
	...
	carl::weight_parameter_function& alpha_mask = CoupledTest.add_alpha_mask("MicroSys",mesh_weight);
	...
	LargeDeformationElasticity non_linear_system(equation_systems_micro,alpha_mask);

	// [MICRO] Set up the physical properties
	libMesh::NonlinearImplicitSystem& elasticity_system_micro
										= add_nonlinear_elasticity(equation_systems_micro);

	elasticity_system_micro.nonlinear_solver->residual_object = &non_linear_system;
	elasticity_system_micro.nonlinear_solver->jacobian_object = &non_linear_system;

	// [MICRO] Set up the nonlinear solver parameters ...

	// ... defined by libMesh
	equation_systems_micro.parameters.set<libMesh::Real>         ("nonlinear solver absolute residual tolerance") = input_params.nonlinear_abs_tol;
	equation_systems_micro.parameters.set<libMesh::Real>         ("nonlinear solver relative residual tolerance") = input_params.nonlinear_rel_tol;
	equation_systems_micro.parameters.set<unsigned int> ("nonlinear solver maximum iterations")          = input_params.nb_of_nonlinear_max_its;

	// ... defined by us
	equation_systems_micro.parameters.set<int> ("nb_nonlinear_steps") = input_params.nb_of_nonlinear_solves;
...
//	CoupledTest.print_matrix_mediator_info("MicroSys");
	...
	CoupledTest.set_LATIN_nonlinear_solver("MicroSys","Elasticity","NonlinearElasticity",
	...
	CoupledTest.solve_LATIN_nonlinear("MicroSys","NonlinearElasticity","Elasticity",input_params.LATIN_convergence_output);
	...
	system_names_micro.insert("NonlinearElasticity");
