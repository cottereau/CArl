...
void anisotropic_elasticity_tensor_cubic_sym::set_parameters(libMesh::EquationSystems& es, std::string& physicalParamsFile,
		double& meanE, double& meanMu)
{
	...
	for ( ; el != end_el; ++el)
	{
		...
		{
			physical_param_system.solution->set(dof_index, m_domain_to_vec_map[currentSubdomain]);
		}
	}
...
void anisotropic_elasticity_tensor_cubic_sym::generate_rotation_matrices()
{
